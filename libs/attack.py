...
def report_profile_attack(username, proxy):
    ...
    try:
        ...
        print_error("Ocorreu um erro de conexão! (Erro de Requisição ao Facebook)")
        return

    if (res.status_code != 200):
        print_error("Ocorreu um erro de conexão! (CÓDIGO DE STATUS:", res.status_code, ")")
        return

    if ('["_js_datr","' not in res.text):
        print_error("Ocorreu um erro de conexão! (Erro de Cookie _js_datr)")
        return
    
    try:
        js_datr = res.text.split('["_js_datr","')[1].split('",')[0]
    except:
        print_error("Ocorreu um erro de conexão! (Erro de Análise de Cookie)")
        return
    ...
    try:
        ...
        print_error("Ocorreu um erro de conexão! (Erro de Requisição ao Instagram)")
        return
    ...
    if ("datr" not in res.cookies.get_dict()):
        print_error("Ocorreu um erro de conexão! (Erro de Cookie datr)")
        return
    
    if ('["LSD",[],{"token":"' not in res.text):
        print_error("Ocorreu um erro de conexão! (Erro de Cookie LSD)")
        return

    if ('"__spin_r":' not in res.text):
        print_error("Ocorreu um erro de conexão! (Erro de Cookie __spin_r)")
        return

    if ('"__spin_b":' not in res.text):
        print_error("Ocorreu um erro de conexão! (Erro de Cookie __spin_b)")
        return

    if ('"__spin_t":' not in res.text):
        print_error("Ocorreu um erro de conexão! (Erro de Cookie __spin_t)")
        return

    if ('"server_revision":' not in res.text):
        print_error("Ocorreu um erro de conexão! (Erro de Cookie server_revision)")
        return

    if ('"hsi":' not in res.text):
        print_error("Ocorreu um erro de conexão! (Erro de Cookie hsi)")
        return
